...
const saveAvatar = async () => {
    ...
    const file = fileInput.value.input.files[0] // 所有的input(類型是file的)都有支持 多個同時上傳
    if (!file) {
        toastError({ title: 'Select a file to upload first' })
        return
    }
    console.log(file)
    const fileExt = file.name.split('.').pop() // 前綴辭去掉後 剩下副檔名
    const fileName = `${Math.random()}.${fileExt}` // 自我控制 檔案名稱
